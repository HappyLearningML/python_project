...
def _load_gt_file(hypes, data_file=None):
    ...
    for epoche in itertools.count():
        ...
        for file in files:
            ...
            if 'KITTI' not in image_file:    
                gt_image = scp.misc.imread(gt_image_file)
                gt_image = gt_image.reshape(gt_image.shape[0],gt_image.shape[1],1)
                if 'Apollo' in image_file:
                    gt_bg = np.all(gt_image != 49, axis=2)
                    gt_road = np.all(gt_image == 49, axis=2)
                elif 'bdd100k' in image_file:
                    gt_bg = np.all(gt_image != 0, axis=2)
                    gt_road = np.all(gt_image == 0, axis=2)   
                elif 'cityscapes' in image_file:
                    gt_bg = np.all(gt_image != 7, axis=2)
                    gt_road = np.all(gt_image == 7, axis=2)                     
                gt_image = np.stack((gt_bg, gt_road), axis=2)                
            else:
                gt_image = scp.misc.imread(gt_image_file, mode='RGB')
                gt_bg = np.all(gt_image == [255,0,0], axis=2)
                gt_road = np.all(gt_image == [255,0,255], axis=2)
                gt_image = np.stack((gt_bg, gt_road), axis=2)  
...
def _make_data_gen(hypes, phase, data_dir):
    ...
    road_color = hypes['data']['road_color']
    background_color = hypes['data']['background_color']
