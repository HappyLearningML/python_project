...
def set_dirs(hypes, hypes_fname):
    ...
    if 'data_dir' not in hypes['dirs']:
        if 'TV_DIR_DATA' in os.environ:
            ...
            #data_dir = '/ssd/anzhi-training-dataset/baiduApolloScape/'
            #data_dir = '/ssd/anzhi-training-dataset/KITTI/'
            #data_dir = '/home/yliu/data/bdd100k/'
            #data_dir = '/home/yliu/code/SCNN/data/CULane/'
            data_dir = '/'
...
def load_modules_from_logdir(logdir, dirname="model_files", postfix=""):
    ...
    """
    model_dir = os.path.join(logdir, dirname)                                # .\RUNS\KittiSeg_pretrained\model_files
...
def load_hypes_from_logdir(logdir, subdir="model_files", base_path=None):
    ...
    hypes['dirs']['output_dir'] = os.path.realpath(logdir)                    # .\RUNS\KittiSeg_pretrained
    hypes['dirs']['image_dir'] = os.path.join(hypes['dirs']['output_dir'],
                                              'images')                       # .\RUNS\KittiSeg_pretrained\images

    if base_path is not None:
        hypes['dirs']['base_path'] = os.path.realpath(base_path)              # .\hypes

    _add_paths_to_sys(hypes)                                                  # .\incl

   
    if 'dirs' not in hypes or 'data_dir' not in hypes['dirs']:
        if 'TV_DIR_DATA' in os.environ:
            data_dir = os.environ['TV_DIR_DATA']
        else:
            data_dir = 'DATA'

        hypes['dirs']['data_dir'] = data_dir                                      # .\data
...
_set_cfg_value('plugin_dir',
               ...
               cfg)
_set_cfg_value('step_show', 'TV_STEP_SHOW', 200, cfg)
_set_cfg_value('step_eval', 'TV_STEP_EVAL', 15000, cfg)
_set_cfg_value('step_write', 'TV_STEP_WRITE', 100, cfg)
_set_cfg_value('max_to_keep', 'TV_MAX_KEEP', 1, cfg)
