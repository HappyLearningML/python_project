...
from tf_utils import tensorvision_core as core
...
def _print_eval_dict(eval_names, eval_results, prefix=''):
    ...
    print_str = ', '.join([nam + ": %.2f" for nam in eval_names])  # python 3.X
...
def run_training(hypes, modules, tv_graph, tv_sess, start_step=0):
    ...
    for step in xrange(start_step, hypes['solver']['max_steps']):
        ...
        if step % display_iter:
            ...
            #print(['training step: ',step,'time: ',time.time()])
...
def _print_training_status(hypes, step, loss_value, start_time, lr):
    ...
    duration = (time.time() - start_time) / hypes['logging']['display_iter']
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False,allow_soft_placement=True,gpu_options=gpu_options)) as sess:
        ...
        with tf.name_scope('Validation'):
            ...
            if hypes['jitter']['reseize_image']:
                image.set_shape([1, hypes['jitter']['image_height'], hypes['jitter']['image_width'], 3])
            else:
                image.set_shape([1, None, None, 3])
            inf_out,_logits = core.build_inference_graph(hypes, modules,
                                                 image=image)
