...
from segmentation_models.FC_DenseNet_Tiramisu import build_fc_densenet
from segmentation_models.Encoder_Decoder import build_encoder_decoder
from segmentation_models.RefineNet import build_refinenet
from segmentation_models.FRRN import build_frrn
from segmentation_models.MobileUNet import build_mobile_unet
from segmentation_models.PSPNet import build_pspnet
from segmentation_models.GCN import build_gcn
from segmentation_models.DeepLabV3 import build_deeplabv3
from segmentation_models.DeepLabV3_plus import build_deeplabv3_plus
from segmentation_models.AdapNet import build_adaptnet
from segmentation_models.custom_model import build_custom
from segmentation_models.DenseASPP import build_dense_aspp
from segmentation_models.DDSC import build_ddsc
from segmentation_models.BiSeNet import build_bisenet
...
def download_checkpoints(model_name):
    subprocess.check_output(["python", "sys_tools/get_pretrained_checkpoints.py", "--model=" + model_name])



def build_model(model_name, frontend, net_input, num_classes, crop_width, crop_height, is_training=True):
	...
	if "ResNet50" == frontend and not os.path.isfile("pretrained_models/resnet_v2/resnet_v2_50.ckpt"):
	    download_checkpoints("ResNet50")
	if "ResNet101" == frontend and not os.path.isfile("pretrained_models/resnet_v2/resnet_v2_101.ckpt"):
	    download_checkpoints("ResNet101")
	if "ResNet152" == frontend and not os.path.isfile("pretrained_models/resnet_v2/resnet_v2_152.ckpt"):
	    download_checkpoints("ResNet152")
	if "MobileNetV2" == frontend and not os.path.isfile("pretrained_models/mobilenet_v2/mobilenet_v2.ckpt.data-00000-of-00001"):
	    download_checkpoints("MobileNetV2")
	if "InceptionV4" == frontend and not os.path.isfile("pretrained_models/inception/inception_v4.ckpt"):
	...
	if model_name == "FC-DenseNet56" or model_name == "FC-DenseNet67" or model_name == "FC-DenseNet103":
	    ...
	    network, init_fn = build_pspnet(net_input, label_size=[crop_height, crop_width], preset_model = model_name, frontend=frontend, num_classes=num_classes, is_training=is_training, pretrained_dir="pretrained_models/resnet_v2")
	elif model_name == "GCN":
	    ...
	    network, init_fn = build_deeplabv3_plus(net_input, preset_model = model_name, frontend=frontend, num_classes=num_classes, is_training=is_training, pretrained_dir="pretrained_models/resnet_v2")
		...
		network, init_fn = build_bisenet(net_input, preset_model = model_name, frontend=frontend, num_classes=num_classes, is_training=is_training, pretrained_dir="pretrained_models/resnet_v2")
