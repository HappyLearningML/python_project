...
class VideoReader(object):
    ...
    def __init__(self, filename, cache_capacity=10):
        path.check_file_exist(filename, 'Video file not found: ' + filename)
    ...
    def cvt2frames(self,
        ...
        if show_progress:
            progressbar.track_progress(write_frame, range(file_start,
                                              file_start + task_num))
        else:
...
def frames2video(frame_dir,
    ...
    if end == 0:
        ...
        end = len([name for name in path.scandir(frame_dir, ext)])
    first_file = osp.join(frame_dir, filename_tmpl.format(start))
    path.check_file_exist(first_file, 'The start frame not found: ' + first_file)
    ...
    if show_progress:
        progressbar.track_progress(write_frame, range(start, end))
    else:
