...
from images_tools import opencv_info
from sys_tools import path
from images_tools import colorspace, geometry, image_resize
...
if not opencv_info.USE_OPENCV2:
...
def imread(img_or_path, flag='color'):
    ...
    if isinstance(img_or_path, np.ndarray):
        return img_or_path
    elif path.is_str(img_or_path):
        flag = imread_flags[flag] if path.is_str(flag) else flag
        path.check_file_exist(img_or_path,
...
class TestImageIO(object):
    ...
    def test_imread(self):
        ...
        img = imread(self.img_path, 'grayscale')
        assert img.shape == (300, 400)
        img = imread(self.gray_img_path)
        assert img.shape == (300, 400, 3)
        img = imread(self.gray_img_path, 'unchanged')
        assert img.shape == (300, 400)
        img = imread(img)
        assert_array_equal(img, imread(img))
    ...
    def test_gray2bgr(self):
        ...
        out_img = colorspace.gray2bgr(in_img)
    ...
    def test_bgr2rgb(self):
        in_img = np.random.rand(10, 10, 3).astype(np.float32)
        out_img = colorspace.bgr2rgb(in_img)
    ...
    def test_rgb2bgr(self):
        in_img = np.random.rand(10, 10, 3).astype(np.float32)
        out_img = colorspace.rgb2bgr(in_img)
        assert out_img.shape == in_img.shape
        assert_array_equal(out_img[..., 0], in_img[..., 2])
        assert_array_equal(out_img[..., 1], in_img[..., 1])
        assert_array_equal(out_img[..., 2], in_img[..., 0])
    ...
    def test_bgr2hsv(self):
        in_img = np.random.rand(10, 10, 3).astype(np.float32)
        out_img = colorspace.bgr2hsv(in_img)
    ...
    def test_bgr2hls(self):
        in_img = np.random.rand(10, 10, 3).astype(np.float32)
        out_img = colorspace.bgr2hls(in_img)
        argmax = in_img.argmax(axis=2)
    ...
    def test_imrescale(self):
        ...
        with pytest.raises(ValueError):
            image_resize.imrescale(self.img, -0.5)
        with pytest.raises(TypeError):
            image_resize.imrescale(self.img, [100, 100])

    def test_imflip(self):
        # test horizontal flip (color image)
        img = np.random.rand(80, 60, 3)
        h, w, c = img.shape
        flipped_img = geometry.imflip(img)
        ...
        flipped_img = geometry.imflip(img, direction='vertical')
        assert flipped_img.shape == img.shape
        for i in range(h):
            for j in range(w):
                for k in range(c):
    ...
    def test_imcrop(self):
        ...
        # crop one bbox
        patch = geometry.imcrop(self.img, bboxes[0, :])
        patches = geometry.imcrop(self.img, bboxes[[0], :])
        ...
        patches = geometry.imcrop(self.img, bboxes)
        ...
        patches = geometry.imcrop(self.img, bboxes, 1.2)
        for i in range(len(patches)):
            ref_patch = np.load(patch_path + '/scale_{}.npy'.format(i))
            self.assert_img_equal(patches[i], ref_patch)

        # crop with scaling and padding
        patches = geometry.imcrop(self.img, bboxes, 1.2, pad_fill=[255, 255, 0])
        for i in range(len(patches)):
            ref_patch = np.load(patch_path + '/pad_{}.npy'.format(i))
            self.assert_img_equal(patches[i], ref_patch)
        patches = geometry.imcrop(self.img, bboxes, 1.2, pad_fill=0)
        for i in range(len(patches)):
    ...
    def test_impad(self):
        ...
        padded_img = geometry.impad(img, (15, 12), 0)
        ...
        padded_img = geometry.impad(img, (15, 12, 3), [100, 110, 120])
        assert_array_equal(img, padded_img[:10, :10, :])
        assert_array_equal(
    ...
    def test_imrotate(self):
        ...
        assert_array_equal(geometry.imrotate(img, 90, center=(0, 0)), img_r)
        img_r = np.array([[255, 6, 2, 255], [255, 7, 3, 255]])
        assert_array_equal(geometry.imrotate(img, 90, border_value=255), img_r)
        img_r = np.array([[5, 1], [6, 2], [7, 3], [8, 4]])
        assert_array_equal(geometry.imrotate(img, 90, auto_bound=True), img_r)

        with pytest.raises(ValueError):
            geometry.imrotate(img, 90, center=(0, 0), auto_bound=True)
    ...
    def test_iminvert(self):
        ...
        img_r = np.array([[255, 127, 0], [254, 128, 1], [253, 126, 2]],
                         dtype=np.uint8)
        assert_array_equal(colorspace.iminvert(img), img_r)
