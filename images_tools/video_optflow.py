...
from images_tools import quantization, image_io
from sys_tools import misc

from images_tools._ext import flow_warp_c
...
def flowread(flow_or_path, quantize=False, concat_axis=0, *args, **kwargs):
    ...
    if isinstance(flow_or_path, np.ndarray):
        if (flow_or_path.ndim != 3) or (flow_or_path.shape[-1] != 2):
            raise ValueError('Invalid flow with shape {}'.format(
                flow_or_path.shape))
        return flow_or_path
    elif not misc.is_str(flow_or_path):
        raise TypeError(
    ...
    if not quantize:
        ...
        cat_flow = image_io.imread(flow_or_path, flag='unchanged')
        if cat_flow.ndim != 2:
            raise IOError(
...
def flowwrite(flow, filename, quantize=False, concat_axis=0, *args, **kwargs):
    ...
    if not quantize:
        ...
        image_io.imwrite(dxdy, filename)
...
def quantize_flow(flow, max_val=0.02, norm=True):
    ...
    flow_comps = [
        quantization.quantize(d, -max_val, max_val, 255, np.uint8) for d in [dx, dy]
    ]
...
def dequantize_flow(dx, dy, max_val=0.02, denorm=True):
    ...
    dx, dy = [quantization.dequantize(d, -max_val, max_val, 255) for d in [dx, dy]]
