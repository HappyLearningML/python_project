...
from images_tools import video_io, video_processing
...
class TestCache(object):
    ...
    def test_init(self):
        with pytest.raises(ValueError):
            video_io.Cache(0)
        cache = video_io.Cache(100)
        assert cache.capacity == 100
        assert cache.size == 0

    def test_put(self):
        cache = video_io.Cache(3)
        for i in range(1, 4):
...
class TestVideo(object):
    ...
    def test_load(self):
        v = video_io.VideoReader(self.video_path)
        assert v.width == 294
        assert v.height == 240
        assert v.fps == 25
    ...
    def test_iterator(self):
        cnt = 0
        for img in video_io.VideoReader(self.video_path):
            cnt += 1
            assert img.shape == (240, 294, 3)
        assert cnt == self.num_frames

    def test_with(self):
        with video_io.VideoReader(self.video_path) as v:
            assert v.opened
        assert not v.opened
    ...
    def test_frames2video(self):
        ...
        video_io.frames2video(frame_dir, out_filename)
        v = video_io.VideoReader(out_filename)
        assert v.fps == 30
        assert len(v) == self.num_frames

        video_io.frames2video(
            frame_dir,
            out_filename,
            fps=25,
            start=10,
    ...
    def test_cut_concat_video(self):
        ...
        video_processing.cut_video(self.video_path, part1_file, end=3, vcodec='h264')
        video_processing.cut_video(self.video_path, part2_file, start=3, vcodec='h264')
        v1 = video_io.VideoReader(part1_file)
        v2 = video_io.VideoReader(part2_file)
        assert len(v1) == 75
        assert len(v2) == self.num_frames - 75

        out_file = osp.join(tempfile.gettempdir(), '.mmcv_test.mp4')
        video_processing.concat_video([part1_file, part2_file], out_file)
        v = video_io.VideoReader(out_file)
        assert len(v) == self.num_frames
    ...
    def test_resize_video(self):
        out_file = osp.join(tempfile.gettempdir(), '.mmcv_test.mp4')
        video_processing.resize_video(self.video_path, out_file, (200, 100), quiet=True)
        v = video_io.VideoReader(out_file)
        assert v.resolution == (200, 100)
        os.remove(out_file)
        video_processing.resize_video(self.video_path, out_file, ratio=2)
        v = video_io.VideoReader(out_file)
        assert v.resolution == (294 * 2, 240 * 2)
        os.remove(out_file)
        video_processing.resize_video(self.video_path, out_file, (1000, 480), keep_ar=True)
        v = video_io.VideoReader(out_file)
        assert v.resolution == (294 * 2, 240 * 2)
        os.remove(out_file)
        video_processing.resize_video(
